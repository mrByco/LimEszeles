...
namespace limesz_app.Misc
{
    public class AzureStorage : IAzureStorage
    {
        ...
        public async Task<BlobResponseDto> UploadAsync(byte[] blob, string filename)
        {
            ...
            try
            {
                ...
                await using (Stream? data = new MemoryStream())
                {
                    data.Write(blob);
                    data.Position = 0;
        ...
        public async Task<BlobResponseDto> UploadAsync(Stream stream, string blobName)
        {
